...
const SIMPLE_ABSOLUTE = [
  ...
  /^(?:what|how) (?:is|does|are|do) (?:a |an |the )?(?:sharpe|sortino|alpha|beta|tvl|fdv|rsi|macd|moving average|bollinger|volatility score|trend score|momentum score|liquidity score|trust score|sentiment score|dse|arcs|signal strength|regime|market regime)/i,
  /^(?:explain|define)\s+(?:the\s+)?(?:sharpe|sortino|alpha|beta|tvl|fdv|rsi|macd|moving average|bollinger|volatility|trend|momentum|liquidity|trust|sentiment|dse|arcs|signal strength|regime|market regime)/i,
  ...
  /^what(?:'s| is) the difference between (?:alpha and beta|btc and eth|eth and btc|layer 1 and layer 2|layer 2 and layer 1|defi and cefi|cefi and defi|growth and value|active and passive|trading and investing|investing and trading|saving and investing|investing and saving|futures and options|options and futures|spot and futures|dex and cex|cex and dex|proof of work and proof of stake|proof of stake and proof of work)\b/i,
  // 7. Educational "should I" questions without tickers — beginner investing, not portfolio strategy
  /^should i (?:invest in (?:crypto|bitcoin|ethereum|defi|nfts|l1|l2)|start (?:investing|trading)|buy (?:crypto|bitcoin|stablecoins)|average down|hold stablecoins|invest during)\b/i,
  // 8. Educational "how do I" / "how should I" / "how much should I" questions without tickers
  /^how (?:do|should|can|much should) i (?:start investing|begin investing|invest|buy (?:my first|a) crypto|open a wallet|choose (?:crypto|a exchange)|find (?:good crypto|small[- ]cap)|track (?:my crypto portfolio|crypto portfolio)|rebalance (?:my crypto portfolio|a crypto portfolio)|reduce (?:portfolio risk|risk)|diversify|avoid (?:emotional|panic)|average down|build (?:a |my )?(?:long[- ]term |passive )?(?:crypto portfolio|income)|know if a crypto is (?:undervalued|overvalued)|invest in (?:each|every|a single|one) crypto)\b/i,
  ...
  /^what should i do (?:when|if|during) (?:my crypto (?:fall|drop|crash)|the market (?:fall|drop|crash|correct)|a (?:market )?(?:crash|correction|downturn))\b/i,
  ...
  // 13. Exchange / platform / wallet choice — educational, no analytical value
  /^which (?:exchange|platform|app|wallet|dex) (?:is|are) (?:best|good|recommended|better|safer)\b/i,
  // 14. Educational "what on-chain metrics / indicators" — definitional lists
  /^what (?:on-chain |key |important )?(?:metrics?|indicators?|signals?) (?:should|do|can|must) i (?:look at|check|use|consider|track)\b/i,
  // 15. Simple "is X safe / good / risky" questions — no analytical signals
  /^is (?:crypto market|investing|trading|crypto|bitcoin) (?:investing |trading )?(?:safe|good|risky|worth it|better|recommended)\b/i,
];
...
const COMPLEX_PATTERNS = [
  ...
  // "difference between TICKER and TICKER" = COMPLEX; "difference between BTC and ETH" = MODERATE (handled by MODERATE_SIGNALS below)
  ...
  /cross[- ](?:asset|sector|market|chain)/i,
  ...
  /(?:factor|sector|token|narrative)\s+(?:rotation|alignment|exposure|tilt)/i,
  ...
  /\b(?:inflation|stagflation|recession|soft\s+landing|hard\s+landing|rate\s+hike|rate\s+cut)\s+(?:impact|scenario|hedge|play|trade)/i,
...
const MODERATE_SIGNALS = [
  /\b[A-Z]{2,8}\b/,                          // contains an uppercase ticker (BTC-USD, ETH-USD)
  ...
  /\b(?:valuation|tvl|fdv|market cap|fully diluted|undervalued|overvalued|fairly valued|on-chain|whale activity|exchange flow|supply distribution|token unlock|vesting)\b/i,
...
// Catches lowercase tickers like "btc-usd", "eth-usd", "sol-usd" that the uppercase regex misses.
...
export function classifyQuery(
  ...
  // BUT only if no analytical signals are present (prevents "What are the key drivers for BTC-USD" → SIMPLE)
  if (SIMPLE_PATTERNS.some((p) => p.test(trimmed))) {
    if (!hasModerateSignal(trimmed)) {
      return "SIMPLE";
    }
