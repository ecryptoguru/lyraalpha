    ...
    return (
      <div className="fixed bottom-6 right-4 z-50 sm:right-6">
        <button
          ...
          disabled
          className="group relative flex h-16 w-16 items-center justify-center rounded-full border border-warning/30 bg-white text-warning opacity-80 shadow-[0_16px_48px_rgba(245,158,11,0.12)] transition-all dark:border-warning/22 dark:bg-[#0b1322] dark:text-primary/60 dark:shadow-[0_16px_48px_rgba(0,0,0,0.42)]"
        >
  ...
  return (
    <div className="fixed bottom-6 right-4 z-50 sm:right-6">
      ...
      >
        ...
        <span className="absolute inset-0 rounded-full bg-warning3377" style={{ animation: "ping 1s cubic-bezier(0,0,0.2,1) 7" }} />
        {hasUnread ? (
          <span className="absolute -right-1 -top-1 h-4 w-4 rounded-full border-2 border-white bg-danger animate-pulse dark:border-[#0b1322]" />
        ) : null}
      </button>
    </div>
  );
