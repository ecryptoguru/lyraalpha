...
export function PointsDisplay({ compact = false, variant = "default" }: PointsDisplayProps) {
  ...
  if (compact) {
    return (
      ...
      >
        ...
        <span className="text-sm font-bold text-[#FFD700]">{points.currentXp}</span>
      </button>
    );
  }
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-3 relative z-10">
        ...
        <div className="flex items-center gap-1 shrink-0">
          <span className="text-sm font-bold text-[#FFD700] tabular-nums leading-none">
            {points.currentXp}
          </span>
          <span className="text-[10px] font-bold text-[#FFD700]/70">XP</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between relative z-10">
        ...
        <button
          onClick={() => setShowModal(true)}
          className="shrink-0 text-[9px] font-bold uppercase tracking-wider text-[#FFD700] hover:text-[#FFD700] transition-colors bg-[#FFD700]/10 hover:bg-[#FFD700]/20 px-2 py-1 rounded ml-2"
        >
          Redeem
        </button>
      </div>
  ...
  return (
    <>
      <div className="rounded-2xl border border-border/75 dark:border-white/5 bg-white/80 dark:bg-white/5 p-4 backdrop-blur-2xl shadow-lg relative overflow-hidden group hover:border-[#FFD700]/25 transition-colors duration-500">
        <div className="absolute inset-0 bg-linear-to-br from-[#FFD700]/10 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
        {innerContent}
      </div>
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-4xl font-bold text-[#FFD700]">{points?.currentXp || 0}</p>
                  ...
                  return (
                    <button
                      ...
                      className={`w-full p-4 rounded-2xl border flex items-center justify-between transition-all ${
                        canRedeem
                          ? "border-[#FFD700]/30 bg-[#FFD700]/10 hover:bg-[#FFD700]/20 cursor-pointer"
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="font-bold text-[#FFD700]">{option.xpCost} XP</p>
                  ...
                  <div className="space-y-1">
                    {history.slice(0, 5).map((tx) => (
                      <div key={tx.id} className="flex items-center justify-between text-xs py-2 border-b border-white/5">
                        ...
                        <span className={tx.amount > 0 ? "text-success font-bold" : "text-danger font-bold"}>
