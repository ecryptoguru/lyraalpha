...
import type { ETFRiskResult, RiskLevel } from "@/lib/engines/etf-risk";

interface ETFRiskCardProps {
  risk: ETFRiskResult;
  className?: string;
}
...
export function ETFRiskCard({ risk, className }: ETFRiskCardProps) {
    ...
    <div className={cn("bg-card/60 backdrop-blur-2xl border border-white/5 shadow-xl rounded-3xl border border-white/5 bg-card/60 backdrop-blur-2xl overflow-hidden", className)}>
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="space-y-1">
          ...
          <SubRiskRow label="Tracking Error" level={risk.tracking.level} />
          <SubRiskRow label="Liquidity" level={risk.liquidity.level} />
        ...
        <div className="pt-3 border-t border-border/30 grid grid-cols-2 gap-3">
          {risk.tracking.trackingError != null && (
            <div className="text-center p-2 rounded-2xl bg-background/30">
              <p className="text-sm font-bold text-foreground">{(risk.tracking.trackingError * 100).toFixed(2)}%</p>
              <p className="text-[8px] text-muted-foreground font-bold uppercase">Tracking Error</p>
            </div>
          )}
          {risk.tracking.correlation != null && (
            <div className="text-center p-2 rounded-2xl bg-background/30">
              <p className="text-sm font-bold text-foreground">{risk.tracking.correlation.toFixed(2)}</p>
              <p className="text-[8px] text-muted-foreground font-bold uppercase">Benchmark Corr.</p>
            </div>
          )}
