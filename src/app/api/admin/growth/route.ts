...
import { getGrowthStats, type UsageRange } from "@/lib/services/admin.service";
import { createLogger } from "@/lib/logger";
import { sanitizeError } from "@/lib/logger/utils";
import { apiError } from "@/lib/api-response";

const logger = createLogger({ service: "admin-growth" });
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const stats = await getGrowthStats(range);
    const response = NextResponse.json(stats);
    response.headers.set("Cache-Control", "private, max-age=30, stale-while-revalidate=30");
    return response;
  } catch (error) {
    logger.error({ err: sanitizeError(error) }, "Failed to fetch growth stats");
    return apiError("Failed to fetch growth stats", 500);
  }
}
